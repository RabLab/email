+function ($) { "use strict";
    var TemplateForm = function () {
        this.$preview = $('#email-template-preview')
        ...
        this.$textarea = $('[name="Template[content]"]', this.$form)
        this.$previewContent = $('.preview-content', this.$preview)
        this.codeEditor = $('textarea[name="Template[content]"]', this.$form).closest('.field-codeeditor').data('oc.codeEditor')
    ...
    TemplateForm.prototype.handleChange = function() {
    ...
    TemplateForm.prototype.createIndicator = function() {
        var $previewContainer = $('#email-template-preview').closest('.loading-indicator-container')
        this.$indicator = $('<div class="loading-indicator transparent"><div></div><span></span></div>')
        $previewContainer.prepend(this.$indicator)
    }

    TemplateForm.prototype.update = function() {
        var self = this
    ...
    TemplateForm.prototype.showIndicator = function() {
        this.$indicator.css('display', 'block')
    }

    TemplateForm.prototype.hideIndicator = function() {
        this.$indicator.css('display', 'none')
    }

    TemplateForm.prototype.initPreview = function() {
        prettyPrint()
        this.initImageUploaders()
    }

    TemplateForm.prototype.updateScroll = function() {
        this.$preview.data('oc.scrollbar').update()
    }

    TemplateForm.prototype.initImageUploaders = function() {
        var self = this
    ...
    TemplateForm.prototype.pauseUpdates = function() {
        this.updatesPaused = true
    }

    TemplateForm.prototype.resumeUpdates = function() {
        this.updatesPaused = false
    }

    TemplateForm.prototype.initDropzones = function() {
        ...
        $(document).on('click', '#email-template-preview span.dropzone', function() {
            $('input[type=file].trigger', $(this).parent()).click()
        })

        $(document).on('change', '#email-template-preview input[type=file].trigger', function(e) {
    ...
    TemplateForm.prototype.initFormEvents = function() {
        $(document).on('ajaxSuccess', '#template-form', function(event, context, data){
        ...
        $('#Datepicker-formPublishedAt-input-published_at').triggerOn({
            ...
            trigger: '#Form-form-field-Template-published', 
            triggerType: 'enable'})
    }

    TemplateForm.prototype.initLayout = function() {
        $('#Form-form-tabs-secondary .tab-pane.layout-cell:not(:first-child)').addClass('padded-pane')
    }

    TemplateForm.prototype.replacePlaceholder = function(placeholder, placeholderHtmlReplacement, mdCodePlaceholder, mdCodeReplacement) {
        this.pauseUpdates()
